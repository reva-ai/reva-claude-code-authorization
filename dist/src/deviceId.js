...
exports.readOauthAccountId = readOauthAccountId;
exports.readOsHardwareId = readOsHardwareId;
exports.persistedFallbackId = persistedFallbackId;
...
function readOauthEmail(claudeJsonPath) {
    try {
...
function readOauthAccountId(claudeJsonPath) {
    try {
        const filePath = claudeJsonPath ||
            (process.env.REVA_CLAUDE_JSON_PATH?.length ? process.env.REVA_CLAUDE_JSON_PATH : undefined) ||
            path.join(os.homedir(), '.claude.json');
...
function readWindowsMachineGuid() {
    try {
        const out = (0, node_child_process_1.execFileSync)('reg', ['query', 'HKLM\\SOFTWARE\\Microsoft\\Cryptography', '/v', 'MachineGuid'], { encoding: 'utf8', timeout: 2000 });
...
// across future runs.
function persistedFallbackId(dataDir) {
    if (!dataDir)
...
// for something else.
function resolveMachineId(pluginDataDir) {
    if (!cachedMachineId) {
