...
exports.mapToolToCedar = mapToolToCedar;
...
function looksLikeFile(pathValue) {
...
function literalDirOf(patternValue) {
...
function expandHome(pathValue) {
    ...
    if (pathValue.startsWith('~/'))
        return path.join(os.homedir(), pathValue.slice(2));
    return pathValue;
}
function resolveAgainst(cwd, pathValue) {
...
function toPortableId(absolutePath, cwd) {
...
// to report for those.
function buildAncestorParents(portableId) {
...
function extractBashTarget(command, cwd) {
    const tokens = command.trim().split(/\s+/).slice(1);
    const candidate = tokens.find((t) => t.length > 0 && !t.startsWith('-') && !t.startsWith('$') && !t.startsWith('`') && (t.includes('/') || t.includes('.')));
    if (candidate) {
...
function mapToolToCedar(toolName, toolInput, cwd) {
    if (toolName === 'Bash') {
    ...
    if (toolName === 'Agent' || toolName === 'Task') {
        const subagentType = toolInput.subagent_type || toolInput.subagentType || toolInput.agent_type || 'subagent';
        return {
            actionName: 'spawn',
    ...
    if (toolName.startsWith('mcp__')) {
        ...
        const server = (0, mcpServerIdentity_1.resolveMcpServerIdentity)(token, process.env.CLAUDE_PLUGIN_DATA);
