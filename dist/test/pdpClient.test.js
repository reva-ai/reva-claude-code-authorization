...
const pdpClient_1 = require("../src/pdpClient");
const baseCfg = {
...
(0, node_test_1.test)('200 always allows by status, even with decision false or malformed JSON', async () => {
    for (const body of [JSON.stringify({ decision: false }), 'not-json']) {
        await withFetch((async () => new Response(body, { status: 200 })), async () => {
            const result = await (0, pdpClient_1.evaluate)(baseCfg, baseRequest, '00-x-y-01');
...
(0, node_test_1.test)('403 remains a blocking policy deny, reads context.reason', async () => {
    let calls = 0;
    await withFetch((async () => {
        calls += 1;
        return new Response(JSON.stringify({ context: { reason: 'policy denied' } }), { status: 403 });
    }), async () => {
        const first = await (0, pdpClient_1.evaluate)(baseCfg, baseRequest, '00-x-y-01');
        const second = await (0, pdpClient_1.evaluate)(baseCfg, baseRequest, '00-x-y-01');
        strict_1.default.equal(first.decision, 'deny');
        strict_1.default.equal(first.status, 403);
        strict_1.default.equal(first.reason, 'policy denied');
...
(0, node_test_1.test)('every 401 error type fails open for that one call, but never persists', async () => {
...
(0, node_test_1.test)('every 500-599 response fails open for that one call, preserving status/type', async () => {
...
(0, node_test_1.test)('non-JSON 5xx uses the stable PDP_SERVER_ERROR fallback and still fails open', async () => {
    await withFetch((async () => new Response('bad gateway', { status: 502 })), async () => {
        const result = await (0, pdpClient_1.evaluate)(baseCfg, baseRequest, '00-x-y-01');
        strict_1.default.equal(result.decision, 'allow');
        strict_1.default.equal(result.inactive, true);
        strict_1.default.equal(result.status, 502);
        strict_1.default.equal(result.errorType, 'PDP_SERVER_ERROR');
    });
});
(0, node_test_1.test)('400 and 413 deny without failing open', async () => {
...
(0, node_test_1.test)('network failure synthesizes 503 PDP_UNAVAILABLE and fails open', async () => {
    await withFetch((async () => {
        ...
        strict_1.default.equal(result.errorType, 'PDP_UNAVAILABLE');
    });
});
(0, node_test_1.test)('timeout synthesizes 504 PDP_TIMEOUT and fails open', async () => {
    await withFetch(((_url, opts) => new Promise((_resolve, reject) => {
        opts.signal.addEventListener('abort', () => reject(new Error('aborted')));
    })), async () => {
        const result = await (0, pdpClient_1.evaluate)({ ...baseCfg, timeoutMs: 10 }, baseRequest, '00-x-y-01');
        strict_1.default.equal(result.decision, 'allow');
        strict_1.default.equal(result.inactive, true);
        strict_1.default.equal(result.status, 504);
        strict_1.default.equal(result.errorType, 'PDP_TIMEOUT');
    });
});
(0, node_test_1.test)('request uses canonical body and raw X-API-Token header', async () => {
    ...
    await withFetch((async (_url, opts) => {
        ...
        await (0, pdpClient_1.evaluate)(baseCfg, baseRequest, '00-x-y-01');
    });
