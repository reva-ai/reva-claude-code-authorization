...
export function isOutsideCodeScope(env: NodeJS.ProcessEnv = process.env): boolean {
...
export function skipOutsideCodeScope(): boolean {
  if (!isOutsideCodeScope()) return false;
  ...
  debugLog('scope: skipping Chat/Cowork runtime; Claude Code authorization is not applicable');
  return true;
}
