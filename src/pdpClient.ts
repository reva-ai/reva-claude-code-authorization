import { CedarRequest, PdpResult, RevaConfig } from './types';
...
async function postToPdp(cfg: RevaConfig, request: CedarRequest, traceparent: string): Promise<PdpHttpResponse> {
...
function handleOperationalStatuses(res: PdpHttpResponse): PdpResult | undefined {
  if (res.status === 401) {
...
function handleTransportFailure(err: any): PdpResult {
...
export async function evaluate(cfg: RevaConfig, request: CedarRequest, traceparent: string): Promise<PdpResult> {
  try {
